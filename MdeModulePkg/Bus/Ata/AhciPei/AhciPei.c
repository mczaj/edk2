...
/**
  Callback for PCI_DEVICE_PPI installation.
  ...
  @retval Status of AHCI controller initialization.
**/
EFI_STATUS
EFIAPI
AtaAhciPciDevicePpiInstallationCallback (
  ...
  )
{
  ...
  Status = AtaAhciInitPrivateData (&PciDevice->PciIo, DevicePath, DevicePathLength);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "Failed to init controller\n"));
  }
...
/**
  Initialize AHCI controllers from PCI_DEVICE_PPI instances.

  @retval EFI_SUCCESS  Devices initialized successfully.
**/
EFI_STATUS
